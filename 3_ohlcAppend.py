""" 
    ...
    -Script 1 --> Simply to show where does the last 720 OHLC starts from with regards to closen OHLC time-window
    ...
    -Script 3 (this one) --> Once we have tha MasterFile, we want to append new candles as we go (this script)
...
def get_master_ohlc_CSV():
    ...
    master_last_date = asset_df.index[-1]      # this is for since later to get OHLC
    print (f"\n since for append script {master_last_date}")
...
def export_new_master_ohlc(master_ohlc_df):
    ...
    ''' in case we wanted to export to another file '''
    #now = datetime.now()
    #timeframe = '15min'
    #filename = str(now.year) + str(now.month) + str(now.day) + str(now.hour) + str(now.minute) + "_" + timeframe
    #filename = str(now.year) + "/" + str(now.month) + "/" + str(now.day) + "_" + timeframe + ".csv"
    #print()
    #print (filename)
    #g = input("now  .... Press any key : ")

    #we might want to export to a different file to keep a copy of the original version 
    path_csv = './data/'
    name_csv = 'master_ohlc_appended.csv'
    OHLC_csv = path_csv + name_csv
